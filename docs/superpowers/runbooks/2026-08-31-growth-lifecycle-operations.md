...
| `GROWTH_DATABASE_ENVIRONMENT`                                                                   | Resend webhook routes                            | yes                            | exactly `test`, `preview`, or `production`; values must match within a running pair           |
| `DELIVERY_ENVIRONMENT`                                                                          | no                                               | yes                            | exactly `test`, `preview`, or `production`; must match `GROWTH_DATABASE_ENVIRONMENT`          |
...
| `GROWTH_PUBLIC_ACTION_ORIGIN`                                                                   | no                                               | recipient template/action URLs | server-only bare HTTPS Website origin; preview points to preview and production to production |
