...
Expected: exit 0; `git status --porcelain apps/website/content/docs/langgraph/api/api-docs.json` shows it modified (the new `LANGGRAPH_CLIENT_OPTIONS` token). Confirm no `a competing React framework` string appears anywhere in `git diff` (`git diff | grep -i <vendor>` must be empty).
