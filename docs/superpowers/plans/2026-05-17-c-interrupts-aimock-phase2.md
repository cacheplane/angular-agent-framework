...
**Tech Stack:** Playwright 1.x, `aimock` (`llmock` CLI for record mode), LangGraph 1.x `interrupt()` + `Command(resume=...)`, `langgraph_sdk` HTTP API for the record script, Nx `@nx/playwright:playwright` executor, GitHub Actions.
