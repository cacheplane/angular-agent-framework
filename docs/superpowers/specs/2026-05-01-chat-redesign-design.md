...
| 2 | Hybrid styling: component-encapsulated styles + CSS custom properties on `:host` + optional global `chat.css`. | Renders out of the box (Layer 1 + 2), exposes competing-React-framework-style global-class deep-override surface (Layer 3) without making it a setup step. Improves on the competing React framework's React-bundler-side-effect model. |
...
- Avatar block in assistant messages (assistant goes bubble-less, no avatar — matches a competing React framework philosophy).
...
- `<=768px`: hover-revealed message controls become always-visible (matches the competing React framework's `@media` rule).
...
| The new `chat-trace` indented-content pattern may not handle very long tool outputs gracefully. | Content has `max-height: 250px` and internal scroll, matching a competing React framework. |
