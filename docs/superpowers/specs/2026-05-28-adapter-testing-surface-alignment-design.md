...
| **3. LLM provider** | aimock (`libs/e2e-harness`, `aimock` `LLMock`) | Angular app + real `langgraph dev` + real Python graph + real adapter | only the nondeterministic LLM (OpenAI fixtures) | full Playwright e2e |
