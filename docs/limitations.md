# Threadplane — Limitations vs React useStream()
...
**Workaround:** Use the `throttle` option in `provideAgent({ throttle: 50, ... })` to coalesce rapid updates. This applies
...
**Angular behavior:** `injectAgent()` requires a browser environment
...
equivalent. `injectAgent()` is called once. No behavioral impact.
