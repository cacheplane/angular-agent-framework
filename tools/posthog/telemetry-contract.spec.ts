...
async function insightFiles(): Promise<string[]> {
  return (await readdir(INSIGHTS_DIR))
...
test('every insight event is registered in the telemetry contract', async () => {
  ...
  const unregistered = [...referenced]
    .filter((event) => !(event in TELEMETRY_EVENT_CONTRACT))
    .sort();
  assert.deepEqual(
    unregistered,
    [],
  ...
  for (const file of await insightFiles()) {
    ...
    for (const item of insight.events ?? []) {
      ...
      if (!allowed.includes(insight.breakdown)) {
        violations.push(
          `${insight.slug}: ${event} breaks down by ${insight.breakdown}`
        );
      }
    }
  }
    ...
    for (const item of insight.events ?? []) {
      ...
      for (const property of item.properties ?? []) {
        if (property.key && !allowed.includes(property.key)) {
          violations.push(
            `${insight.slug}: ${event} filters by ${property.key}`
          );
        }
      }
    }
...
test('runtime dashboard covers every runtime event exactly once', async () => {
  ...
  for (const tile of dashboard.tiles) {
    ...
    for (const item of insight.events ?? []) {
      if (item.event) {
        coveredEventCounts.set(
          item.event,
          (coveredEventCounts.get(item.event) ?? 0) + 1
        );
      }
    }
  }
...
test('public AgentRuntimeTelemetryEvent union matches the runtime event contract', async () => {
  ...
  const match = runtimeTelemetrySource.match(
    /export type AgentRuntimeTelemetryEvent =([\s\S]*?);/
  );
...
test('sensitive runtime fields are forbidden and never allowed by any event contract', () => {
  ...
  const violations = Object.entries(TELEMETRY_EVENT_CONTRACT).flatMap(
    ([event, contract]) =>
