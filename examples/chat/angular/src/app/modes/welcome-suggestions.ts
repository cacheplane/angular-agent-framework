...
export interface WelcomeSuggestion {
  /**
   * Stable key for `?featured=`. Deliberately NOT derived from the label at
   * runtime: the homepage links to these ids, so renaming a label must not
   * silently break a link. Change an id only if you also update its consumers.
   */
  readonly id: string;
...
export const MORE_SUGGESTIONS: readonly WelcomeSuggestion[] = [
  ...
  {
    id: 'generative-ui-feedback-form',
  ...
  {
    id: 'generative-ui-settings-card',
  ...
  {
    id: 'generative-ui-media-product',
  ...
  {
    id: 'generative-ui-booking-surface',
  ...
  {
    id: 'smoke-interactive-form-kitchen',
 ...
 */
export function suggestionsForAppMode(
  appModeOn: boolean,
  featuredId?: string,
): {
  ...
  const pool = appModeOn
    ? [...ITINERARY_SUGGESTIONS, ...FEATURED_SUGGESTIONS, ...MORE_SUGGESTIONS]
    : [...FEATURED_SUGGESTIONS, ...MORE_SUGGESTIONS];

  // `featuredId` arrives from a URL, so it is untrusted input. Treat it as a
  // KEY, never as content: an id we do not recognise falls back to the default
  // rather than rendering anything the link supplied. That is what stops a
  // crafted link from putting arbitrary words inside the demo UI.
  const requested = featuredId ? pool.find((s) => s.id === featuredId) : undefined;
  const featured = requested ?? pool[0];

  return { featured, more: pool.filter((s) => s.id !== featured.id) };
}
