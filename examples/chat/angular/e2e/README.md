...
Cross-stack E2E harness for the chat example. A repository-owned fixture server provides deterministic OpenAI-compatible responses; the Python LangGraph dev server is launched with `OPENAI_BASE_URL` pointed at it; Playwright drives the Angular `/embed` route in real Chromium.
