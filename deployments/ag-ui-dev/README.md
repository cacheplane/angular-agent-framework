...
The generator reads `libs/cockpit-registry/src/lib/capability-registry.ts`, filters to
`product === 'ag-ui'` and `product === 'runtimes'` entries with a `pythonDir`,
stages each into `deps/<topic>/`, and writes one framework-appropriate mount
(`add_langgraph_fastapi_endpoint`, `add_agent_framework_fastapi_endpoint`, or
