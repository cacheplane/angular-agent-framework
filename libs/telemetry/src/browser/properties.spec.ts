...
describe('browser properties', () => {
  it('detects local hosts for opt-in development capture', () => {
    ...
    expect(isLocalAnalyticsHost('tplane.example')).toBe(false);
    expect(isLocalAnalyticsHost(undefined)).toBe(false);
  });

  it('requires a token and skips local capture unless explicitly enabled', () => {
    expect(shouldCaptureAnalytics({ token: '', captureLocal: false, host: 'tplane.example' })).toBe(false);
    ...
    expect(shouldCaptureAnalytics({ token: 'ph_test', captureLocal: false, host: 'tplane.example' })).toBe(true);
  });
});
