...
import type { TrackNarrativeAction } from '../../host-services';
...
export function NarrativeDocs({
  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLElement>) => {
    ...
    if (copyCodeBtn) {
      ...
      trackNarrativeAction?.({ capability, surface: 'docs_code_snippet' });
    ...
    if (copyPromptBtn) {
      ...
      trackNarrativeAction?.({ capability, surface: 'agentic_prompt' });
      copyPromptBtn.textContent = 'Copied!';
      setTimeout(() => { copyPromptBtn.textContent = 'Copy prompt'; }, 1500);
      return;
    }
  }, [capability, trackNarrativeAction]);
