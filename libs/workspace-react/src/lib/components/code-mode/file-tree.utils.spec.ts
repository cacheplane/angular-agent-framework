...
describe('buildTree', () => {
  ...
  it('compacts single-child folder chains into one row', () => {
    ...
    expect(
      (tree[0] as { children: TreeNode[] }).children.map((c) => c.label).sort()
    ).toEqual(['app.config.ts', 'planning.component.ts']);
  });
  ...
  it('normalizes leading and trailing slashes in paths', () => {
    const tree = buildTree([
      '/angular/src/app/foo.ts',
      'angular/src/app/bar.ts',
    ]);
    expect(tree).toHaveLength(1);
    const folder = tree[0] as {
      kind: 'folder';
      label: string;
      children: TreeNode[];
    };
    ...
    expect(folder.children.map((c) => c.label).sort()).toEqual([
      'bar.ts',
      'foo.ts',
    ]);
    // FileNode.path must not retain the stripped leading slash
    const foo = folder.children.find((c) => c.label === 'foo.ts') as {
      path: string;
    };
    expect(foo.path).toBe('angular/src/app/foo.ts');
  });

  it('normalizes long slash runs with a linear scan', () => {
    const tree = buildTree([
      `${'/'.repeat(20_000)}file.ts${'/'.repeat(20_000)}`,
    ]);
    expect(tree).toEqual<TreeNode[]>([
      { kind: 'file', path: 'file.ts', label: 'file.ts' },
    ]);
  });
});
