...
import type { DocSection } from '@threadplane/cockpit-shell';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <pre
          className="mt-1.5 text-xs font-mono overflow-x-auto"
          ...
          <div>
            ...
            <table className="params w-full text-sm">
              <thead>
                <tr>
                  <th
                    style={{ textAlign: 'left', color: 'var(--ds-text-muted)' }}
                  >
                    Parameter
                  </th>
                  <th
...
export function ApiMode({ docSections, hasCodeFiles = false }: ApiModeProps) {
  if (docSections.length === 0) {
    return (
      <section
        aria-label="API mode"
        className="grid place-items-center h-full text-sm px-6 text-center"
      >
        ...
        {tsSections.length > 0 ? (
          <div>
            ...
            {tsSections.map((section) => (
              <DocArticle
                key={`${section.sourceFile}:${section.title}`}
                section={section}
              />
            ))}
          </div>
