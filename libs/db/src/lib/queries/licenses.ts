...
export async function getLicensesByCustomerId(db: Db, customerId: string): Promise<License[]> {
  return db.select().from(licenses).where(eq(licenses.stripeCustomerId, customerId));
}
