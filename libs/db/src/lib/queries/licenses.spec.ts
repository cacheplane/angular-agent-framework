...
describe('licenses queries', () => {
  ...
  describe('getLicensesByCustomerId', () => {
    it('returns all rows matching the customer id', async () => {
      await upsertLicense(testDb.db, { ...base, stripeSubscriptionId: 'sub_c1', stripeCustomerId: 'cus_multi' });
      await upsertLicense(testDb.db, { ...base, stripeSubscriptionId: 'sub_c2', stripeCustomerId: 'cus_multi' });
      const rows = await getLicensesByCustomerId(testDb.db, 'cus_multi');
      expect(rows.length).toBeGreaterThanOrEqual(2);
    });
  });
