...
function extractComponentTags(source: string): {
  cleaned: string;
...
function escapeCodeHtml(source: string): string {
  return source
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
...
async function renderSteps(
  ...
  for (const block of allBlocks) {
    if (block.type === 'Step' && resolved.includes(block.placeholder)) {
      ...
      const stepHtml = `<div class="doc-step"><div class="doc-step__indicator"><div class="doc-step__number">${stepNum}</div><div class="doc-step__line"></div></div><div class="doc-step__body"><div class="doc-step__title">${
        block.attrs['title'] ?? `Step ${stepNum}`
      }</div><div class="doc-step__content">${parsedContent}</div></div></div>`;
...
async function parseStepContent(content: string): Promise<string> {
  const stepCodeBlocks: Array<{
    lang: string;
    code: string;
    placeholder: string;
  }> = [];
  ...
  for (const block of stepCodeBlocks) {
    ...
    try {
      highlighted = await codeToHtml(codeToHighlight, {
        lang: block.lang,
        themes: { light: 'github-light', dark: 'tokyo-night' },
      });
    } catch {
      const escaped = escapeCodeHtml(codeToHighlight);
...
function wrapCodeBlock(
  ...
  const langLabel =
    lang !== 'text' ? `<span class="doc-codeblock__lang">${lang}</span>` : '';
  const fileLabel = filename
    ? `<span class="doc-codeblock__file">${filename}</span>`
    : '';
  const header =
    fileLabel || langLabel
...
export async function renderMarkdown(
  ...
  for (const line of source.split('\n')) {
    ...
    let titleStart = 2;
    while (line[titleStart] === ' ' || line[titleStart] === '\t') titleStart++;
    title = line.slice(titleStart).trim();
    if (title) break;
  }
