    ...
    return {
      ...
      html: wrapEmail({
        body: `
          ...
          <p style="font-size:20px;font-weight:700;color:#1a1a2e;margin:0 0 14px;line-height:1.3">LangGraph Angular SDK vs @threadplane/langgraph</p>
          <p style="font-size:14px;color:#555770;line-height:1.7;margin:0 0 24px">The LangGraph JS SDK gives you a streaming client. <strong>@threadplane/langgraph</strong> gives you signal-native state, thread persistence, interrupt flows, and a full test harness — all wired together and optimized for Angular's change detection model. See the full comparison on our product page.</p>
