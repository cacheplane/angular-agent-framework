...
function loadApiDocs(): ApiDocEntry[] {
  const candidates = [
    path.join(process.cwd(), 'apps', 'website', 'content', 'docs/agent', 'api', 'api-docs.json'),
    path.join(process.cwd(), 'content', 'docs/agent', 'api', 'api-docs.json'),
  ];
...
export default async function DocsPage({ params }: { params: Promise<{ slug?: string[] }> }) {
  ...
  return (
    <div className="flex min-h-screen pt-16 overflow-x-hidden" style={{ background: 'var(--gradient-bg-flow)' }}>
      <DocsSearch />
      <DocsSidebar activeSection={section} activeSlug={slug} />
      <div className="flex-1 flex min-w-0" style={{ background: 'rgba(255, 255, 255, 0.85)' }}>
        <div className="flex-1 min-w-0">
          ...
          <MdxRenderer source={doc.content} section={section} slug={slug} title={doc.title} />
          {section === 'api' && (() => {
