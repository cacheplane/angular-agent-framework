...
```
Your Angular App
  ↓ submit({ messages: [userMsg] })
streamResource()
  ↓ HTTP POST to LangGraph Platform
FetchStreamTransport
  ↓ Creates thread, starts run
LangGraph Platform
  ↓ Executes graph nodes
  ↓ Streams SSE events back
FetchStreamTransport
  ↓ Parses events into BehaviorSubjects
streamResource()
  ↓ Converts to Angular Signals via toSignal()
Your Angular App
