---
description: How the AG-UI adapter reduces protocol events into Angular signals, which providers register an agent, and what the adapter deliberately leaves out.
---
...
`toAgent()` subscribes to `source.subscribe({ onRunInitialized, onEvent, onRunFailed })`.
...
- `interrupt` cleared on `RUN_STARTED`, and set by the `CUSTOM` `on_interrupt` event or by a `RUN_FINISHED` carrying `outcome: { type: 'interrupt' }`.
...
### Typed state and several agents at one level

`provideAgent()` has a second overload that takes an `AgentRef` first. The ref carries the state shape from the provider to `injectAgent(ref)`, and it registers the agent under the ref's own token, so more than one agent can be provided at a single injector level:
...
providers: [
  provideAgent(TRIP, { url: '/api/trip' }),
  provideAgent(SUPPORT, { url: '/api/support' }),
];

// component
const trip = injectAgent(TRIP);       // AgUiAgent<TripState>
...
`retry()` re-runs the last submitted input without re-appending it to the message history. It clears `error` and sets loading, and it silently does nothing while a run is already in flight or when no input has been submitted yet.
...
- Interrupts from `CUSTOM` events named `on_interrupt`, or from a `RUN_FINISHED` carrying `outcome: { type: 'interrupt' }`.
