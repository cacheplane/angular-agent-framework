...
| `RUN_FINISHED` (outcome `{ type: 'interrupt' }`) | `interrupt`, `interruptSession`, `status`, `isLoading` | Records the full native batch as pending, projects `{ id, value: { interrupts, runId }, resumable: true }`, and returns to `idle`. In default `auto` mode the native batch takes precedence over compatibility events in either arrival order. |
