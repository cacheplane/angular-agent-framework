...
The second suggestion, "Filter to only the cancelled flights", is the follow-up worth trying. The agent does not author a new layout for it. It calls one data tool again with new arguments, the new results replace that slice of state, and the dashboard already on screen re-renders in place.
...
The tool message becomes a short placeholder and the assistant message takes the specification JSON. On the client the content classifier in `@threadplane/chat` sees content that begins with `{`, classifies it as `json-render`, and mounts `<chat-generative-ui>`, which renders it through `<render-spec>` from `@threadplane/render`.
...
The wiring shows the loop. `agent` runs the tools, `wrap_spec_into_ai` post-processes the result and returns to the agent, and the turn ends through `emit_state`, a short conversational summary, and background title generation.
...
Nothing in this file is specific to generative UI. The state events that carry the dashboard data are emitted by the adapter because the graph returns those fields.
...
The `[store]` input is what makes the dashboard live. `<chat>` forwards only an explicit store to the generative-UI surface; without one, the surface seeds a private store from the specification and backend state never reaches it.
