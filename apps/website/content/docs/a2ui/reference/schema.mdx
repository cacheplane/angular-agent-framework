---
description: The TypeScript model of the A2UI v0.9 protocol: constants, dynamic values, actions, the basic-catalog components, envelopes, and outbound messages.
---

# A2UI Schema

The `@threadplane/a2ui` schema is a TypeScript model of the A2UI v0.9 protocol shapes the framework uses. It is a contract for agent output and custom integrations, but it is not a runtime validator.
...
Note that `Button` has no text prop — its label is a `child` Text component referenced by id.

Several fields are constrained to a fixed enum. Emit one of the listed values — an unknown value is not validated at the protocol layer, but a renderer may ignore it or fall back to a default:
...
`Tabs` is the one container that does not use `A2uiChildren`. Its `tabs` field is an array of `{ title: DynamicString; child: string }` pairs.
