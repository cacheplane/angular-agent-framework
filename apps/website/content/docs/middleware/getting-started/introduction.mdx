...
The same entry point also exports a deduplication surface, for backends that must tolerate the same client-tool result arriving twice:
...
| `extractClientToolResultMessages()` | Extracts serialized client-tool results from LangGraph `ToolMessage`s. |
| `recordClientToolResults()` | Records first-seen results and reports which tool-call ids were redeliveries. |
| `filterDuplicateClientToolResultMessages()` | Removes those redelivered result messages before continuation logic sees them. |
| `lookupClientToolExecutions()` | Reads prior executions back for reload reconciliation. |
