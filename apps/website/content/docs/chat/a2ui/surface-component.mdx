---
description: The A2uiSurfaceComponent that renders one A2UI surface - its inputs and outputs, the built-in handler map, and how validation blocks an event action.
---
...
| `handlers` | `Record<string, (params) => unknown \| Promise<unknown>>` | No | Keyed by A2UI function name. Consulted by the built-in `a2ui:localAction` handler, taking priority over its `openUrl` fallback. The handler map itself is fixed at two keys, so the built-in `a2ui:event` handler cannot be overridden |
| `surfaceFallback` | `Type<unknown> \| undefined` | No | Replaces the built-in `A2uiDefaultFallbackComponent` shown while no spec is available. Applies to the `state` input path only |
...
| `validationError` | `A2uiErrorMessage` | Emitted with code `VALIDATION_FAILED` when a check rule on the surface blocks an event action. The action is not emitted |
