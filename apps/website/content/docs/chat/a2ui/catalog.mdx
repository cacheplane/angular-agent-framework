---
description: Every component in the built-in A2UI catalog - its Angular class, selector, props and defaults, plus how bound inputs write values back.
---
...
| `gap` | `number \| 'small' \| 'medium' \| 'large' \| undefined` | Space between children. A number is a spacing unit rendered as `n * 4` pixels; `small`/`medium`/`large` render as 8/12/16 pixels. Unset falls back to the stylesheet default |
...
<Callout type="info" title="Bound inputs write back through emitBinding()">
The five components below (`TextField`, `CheckBox`, `ChoicePicker`, `DateTimeInput`, `Slider`) do not declare an `emit` prop. A user edit is written back by calling `emitBinding()` against the component's `_bindings` map, which the render engine populates from the path references in the wire payload.
</Callout>
...
| Prop | Type | Description |
|------|------|-------------|
| `url` | `string` | Video source URL. Defaults to the empty string |
...
| Prop | Type | Description |
|------|------|-------------|
| `url` | `string` | Audio source URL. Defaults to the empty string |
