---
description: How chat renders assistant markdown — the streaming node renderer, the renderMarkdown() helper, the 26-node registry, and per-instance node overrides.
---
...
Pass a `ViewRegistry` directly to a `<chat-streaming-md>` via its `[viewRegistry]` input:
