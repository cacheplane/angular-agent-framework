---
description: How chat classifies assistant content as markdown, a json-render spec or A2UI as it streams, and how to drive ContentClassifier and ParseTreeStore directly.
---
...
// Create a classifier instance — no injection context required
...
Parse errors are captured in the `errors` signal and do not crash the rendering pipeline. When a malformed token arrives, the classifier records the error and keeps processing the tokens that follow — partial results keep rendering.
