...
A thread id is a UUID, which makes a poor sidebar label. The `generate_title` node asks a small model to summarize the first human message and writes the result to the thread's `metadata.title`, which is exactly where the thread store looks for a label.
...
`LangGraphThreadsAdapter` is a root-provided service that wraps `client.threads.*` from `@langchain/langgraph-sdk`. It reads its base URL from `LANGGRAPH_THREADS_CONFIG`, and that token is the reason this application config is longer than most.
...
The `<chat>` composition owns the transcript, the input, and the loading and error states, so the template only has to add a list. `<chat-thread-list>` takes the threads to render, the id to highlight, and an action adapter for the row menu.
...
Both calls must run inside an injection context, because `refreshOnRunEnd()` uses an Angular effect.

### Switching and creating

Two handlers close the loop. Selecting a row tells the agent to load that thread and updates the signal; the "+ New" button asks the store for a thread and then activates it.

<ExampleCode file="threads.component.ts" region="switch-and-create" title="threads.component.ts — switching and creating" />

`switchThread(id)` resets the agent's derived state and reloads that thread's server-side messages, while `create()` returns `null` when the request fails, which is why the result is checked before it is used.
