...
The write is idempotent: the node reads the thread first and returns early when a title is already present, so later turns cost nothing. The write itself sits inside a `try`/`except` that swallows failures, so a title write that fails never blocks the run.
