...
The rest of `wrap_spec_into_ai` replaces both messages, keeping their ids so LangGraph's `add_messages` reducer matches and replaces rather than appends.

<ExampleCode file="graph.py" region="rewrite-message-content" title="graph.py — rewriting the message content" />

The tool message becomes a short placeholder and the assistant message takes the specification JSON, stripped of a code fence if the model wrapped one around it. The early return on an already-filled parent message is what stops a second pass from wrapping the same payload twice.
