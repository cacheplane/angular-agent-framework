---
description: The createParseTreeStore() factory that bridges the partial-JSON parser to a Spec signal, plus the per-element accumulation state it exposes.
---
...
**Returns:** `ParseTreeStore`. It creates plain signals, so it can be constructed outside an Angular injection context.
