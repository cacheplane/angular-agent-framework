---
description: The mockAgent() testing helper - its options, the writable signals it exposes, and the call logs tests assert against.
---
...
| `error` | `AgentError` | `undefined` | Initial error value |
...
| `history` | `AgentCheckpoint[]` | `undefined` | Enables the optional writable `history` signal, seeded with these checkpoints |
...
agent.error.set(
  new AgentError({ kind: 'connection', message: 'Connection failed', retryable: true }),
);
agent.messages.set([{
  id: 'm1',
  role: 'assistant',
