---
description: Install @threadplane/chat, register a LangGraph agent provider, and render a streaming chat surface — with a no-backend variant using mockAgent().
---
...
`provideAgent()` is the only provider `<chat>` requires. `provideChat()` exists too, but it only parks values on the `CHAT_CONFIG` token for your own components to read — no built-in component injects it, so adding it changes nothing about what renders.
...
<CardGroup cols={3}>
  ...
  <Card title="Components" href="/docs/chat/components/chat">
    Full primitive and composition reference.
  </Card>
  <Card title="Going to production" href="/docs/chat/getting-started/installation">
    Swap the mock for a real adapter, review the production checklist, and deploy your backend — see [Installation](/docs/chat/getting-started/installation) and [Agent deployment](/docs/langgraph/guides/deployment).
  </Card>
</CardGroup>
