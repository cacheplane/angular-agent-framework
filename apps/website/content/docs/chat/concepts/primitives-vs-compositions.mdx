---
description: The two levels of @threadplane/chat UI — styled compositions like chat and chat-sidebar, versus headless primitives you assemble yourself — and how to choose.
---
...
Three more compositions place chat inside a layout shell:

- `<chat-popup>` — a floating launcher button that opens chat in an overlay. Wraps `<chat>`.
- `<chat-sidebar>` — a docked rail pinned to the edge of the viewport. Wraps `<chat>`.
- `<chat-sidenav>` — a collapsible thread and project navigation panel with a built-in footer launcher. It sits beside a chat surface rather than wrapping one, so you still render `<chat>` yourself.

`<chat-popup>` and `<chat-sidebar>` forward a fixed set of inputs to the inner `<chat>`: `agent`, `views`, `clientTools`, `modelOptions`, and `showModelPicker`, plus the two-way `selectedModel`. Anything else on the `<chat>` API — `store`, `handlers`, `threads` — needs `<chat>` directly.
...
See the [Layout Modes](/docs/chat/guides/layout-modes) guide for when to reach for each. `<chat-sidenav>` renders the built-in footer launcher by default: `[debug]` is already `true`, so pass `[debug]="false"` to suppress it.
