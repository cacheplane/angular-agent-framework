...
The Run tab shows a conversation on the left and a sidebar headed "Tool Calls" on the right. One welcome suggestion, "Check a flight status", asks for the status of flight UA123.

The agent answers by calling `lookup_flight`, and a card named `lookup_flight` appears in the sidebar. It expands while the call is running, and once the result arrives it prints the arguments under Inputs and the flight record under Output.
...
`provideAgent()` registers the agent once for the whole application, and `provideChat({})` registers the chat library's configuration, here left at its defaults. The example passes a factory because it resolves its connection details at runtime from the host that serves the demo.
...
The nouns are singularized at a count of one, so a single grouped `search_*` call reads "Searched 1 site". The aviation tools in the example match none of these prefixes, which is why two airport lookups read "Called get_airport_info 2 times".
