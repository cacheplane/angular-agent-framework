...
The example resolves its connection at runtime because the host that serves the demo decides which runtime is attached; your own application passes `apiUrl` and `assistantId` directly.
...
<Callout type="info" title="A dispatch becomes visible when its child stream starts">
...
The card draws no background, border, or radius of its own. The only border anywhere in this pairing is the left rule on the expanded body, drawn by [`<chat-trace>`](/docs/chat/components/chat-trace); any surrounding chrome comes from the host that places the card.
