...
LangGraph resumes by re-executing the interrupting call, and `interrupt()` returns the resume value rather than pausing a second time. The flight lookup above therefore runs twice. Keep that stretch free of side effects: reading data is safe, charging a card there is not.
</Callout>
...
| `--tplane-chat-warning-text` | The "Agent paused" eyebrow and its dot |
| `--tplane-chat-text` | Payload text and the Edit and Respond button labels |
...
The same module exports the reader both components use:
