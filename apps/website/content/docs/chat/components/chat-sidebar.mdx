---
description: The ChatSidebarComponent docked chat panel - overlay and push-content modes, inputs and outputs, projection slots, and the panel-width token.
---
...
`ChatSidebarComponent` always sets `display: flex` on its host. In push-content mode the projected content is additionally inset by the panel width, so it should be designed to reflow gracefully when its available width is reduced.
</Callout>
...
| Panel title | `[chatSidebarPanelTitle]` | Projected into the sidebar's own header bar, next to the close button. Empty by default |
| Chat header | `[chatHeader]` | Forwarded to the inner `<chat>` window header bar, which is empty by default |
...
```css
chat-sidebar {
  --tplane-chat-sidebar-width-drawer: 420px;
}
```

The same token drives the panel width and, in push-content mode, the inset applied to the projected content. It defaults to `28rem`.
