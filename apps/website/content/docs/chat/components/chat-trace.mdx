...
The component uses the following `--tplane-chat-*` tokens:

| Token | Applied to |
|---|---|
| `--tplane-chat-surface-alt` | Trace row background |
| `--tplane-chat-separator` | Row border |
| `--tplane-chat-radius-card` | Row border radius |
| `--tplane-chat-text` | Label and content |
| `--tplane-chat-text-muted` | Meta text, icon |
| `--tplane-chat-warning-text` | Running state icon |
| `--tplane-chat-success` | Done state icon |
| `--tplane-chat-error-text` | Error state icon |
