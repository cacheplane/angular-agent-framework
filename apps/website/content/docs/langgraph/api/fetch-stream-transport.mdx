---
description: FetchStreamTransport is the production LangGraph transport — it wraps the SDK client, streams runs, joins and cancels them, and loads checkpoint history.
---
...
In most apps you never instantiate `FetchStreamTransport` directly. When no custom transport is configured, the runtime creates one from the resolved `apiUrl`, `onThreadId`, and `clientOptions`.
...
const threadId = signal<string | null>(localStorage.getItem('threadId'));

const transport = new FetchStreamTransport(apiUrl, (id) => {
  threadId.set(id);
...
<Callout type="warning" title="A custom transport has to report the ids it creates">
The runtime only wraps `onThreadId` for the transport it constructs itself. A
transport you pass in owns its own thread creation, so every id it creates must
reach the runtime through the configured `onThreadId` callback or through the
`threadId` signal — as above, where the constructor callback writes both. Report
neither and the runtime keeps sending `null`, so every `submit()` starts a fresh
thread.
</Callout>
...
<CardGroup cols={3}>
  ...
  <Card
    ...
    href="/docs/langgraph/api/mock-stream-transport"
  >
