---
description: How injectAgent() exposes agent state as Angular Signals, and how to derive, combine, and react to them with computed() and effect()
---

# Angular Signals

Angular Signals are the reactive primitive that powers `injectAgent()`. Developers coming from a Python AI or agent background often wonder how Angular handles real-time streaming data, and this page answers that. Every property on a LangGraphAgent is a Signal, so your templates update automatically as tokens arrive — no manual subscriptions, no async pipes, no RxJS boilerplate.

<Callout type="info" title="For Python developers">
Think of Signals like a Python property with built-in change notification. When the value changes, every consumer — templates, computed values, effects — re-evaluates automatically. For anyone who has used Pydantic models with validators that react to field changes, Signals are the Angular equivalent but deeply integrated into the rendering engine.
</Callout>
...
The key insight: Angular knows which Signals a template reads. When those Signals change, it re-renders only the affected parts of the DOM. No diffing the entire tree, no zone.js overhead.

## How `injectAgent` Uses Signals Internally

Under the hood, `injectAgent()` receives Server-Sent Events (SSE) over HTTP and feeds them into RxJS BehaviorSubjects. It then converts those BehaviorSubjects into Angular Signals using `toSignal()`. This is the bridge between the async streaming world and Angular's synchronous reactivity model.

<Tabs>
<Tab label="Internal flow">

```typescript
// Simplified view of what injectAgent does internally:
...
// injectAgent(ref) hands you clean Signals (config comes from provideAgent):
protected readonly chat = injectAgent(CHAT_AGENT);

chat.messages();   // Signal<Message[]>
chat.status();     // Signal<'idle' | 'running' | 'error'>
chat.error();      // Signal<AgentError | undefined>
...
Every `injectAgent()` instance moves through a lifecycle: **idle**, **running** while work is in flight, then back to **idle** when the stream completes (or **error** when it fails). The `status()` Signal reflects each transition in real time, while `isLoading()` is the convenience signal for loading UI.
...
console.log(chat.error());      // AgentError { kind: 'server', retryable: true, status: 500, ... }
console.log(chat.isLoading());  // false
```
</Step>
</Steps>
...
const errorDisplay = computed(() => {
  ...
  if (!err) return null;
  if (err.status === 429) return 'Rate limited. Please wait a moment.';
  if (err.kind === 'auth') return 'Your credentials were rejected.';
  return err.message;
});
...
Angular's new control flow syntax (`@if`, `@for`, `@switch`) works naturally with Signals. The template below handles every lifecycle state.
...
Every component using `injectAgent()` should use `ChangeDetectionStrategy.OnPush`. The reason it works, and why it is efficient, is below.
...
Since `injectAgent()` exposes Signals, condition 3 handles everything. When a new token arrives and `messages()` updates, Angular marks only the components reading that Signal for check — not the entire tree.

```typescript
@Component({
  // Always use OnPush with injectAgent
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
...
High-frequency token streaming puts real pressure on a frontend framework. Signals with OnPush hold up better than the alternatives, for the reasons below.
