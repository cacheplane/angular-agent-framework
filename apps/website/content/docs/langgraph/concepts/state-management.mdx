---
description: How LangGraph thread state reaches Angular Signals — designing the Python state, pinning its TypeScript shape, and keeping UI state separate
---
...
LangGraph Platform owns the state. Angular owns the view. `injectAgent()` is the read-only bridge between them. You never manually sync, serialize, or manage agent state in your Angular code.
</Callout>
...
<Step title="`injectAgent()` sends the request">
...
On the Python side, your agent's state is a `TypedDict`. The fields you define here are exactly what the state type you pin with `createAgentRef<T>()` — and pass to `provideAgent(ref, …)` and `injectAgent(ref)` — exposes in TypeScript. Getting the Python state design right is the most important architectural decision in an agent: everything downstream inherits its shape, so it is worth slowing down for.
...
class AgentState(TypedDict):
    # Annotated[list, add] means: append new items, do not replace
...
When a node returns `{"messages": [new_msg]}`, LangGraph does not replace the messages list — it **calls the reducer** to merge the update. This is what `Annotated[list, add]` means: use Python's `operator.add` to concatenate lists.
...
The TypeScript interface you pin with `createAgentRef<T>()` is your contract with the Python state. Every Python state field maps to a TypeScript property. The types do not need to match exactly — they just need to be compatible with the JSON that LangGraph streams back.
...
export const PROJECT_AGENT = createAgentRef<ProjectState>('project_agent');

// Configure in app.config.ts:
// provideAgent(PROJECT_AGENT, { apiUrl: '...', assistantId: 'project_agent' });

const agent = injectAgent(PROJECT_AGENT);
```
...
// Runtime-neutral message projection for UI
const messages = agent.messages();   // Signal<Message[]>
```

## State Updates During Streaming

The agent does not wait until it is finished to send state updates. It streams partial state updates as each node completes. Your Angular signals update incrementally throughout the run.
...
export class ProjectComponent {
  protected readonly agent = injectAgent(PROJECT_AGENT);
}
```
...
`injectAgent()` uses `toSignal()` internally with an explicit `initialValue`. Signals always have a value — even before the first stream update. You never need to handle `undefined` explicitly for the signal itself, though individual state fields may be `null` until the agent populates them.
</Callout>
...
**Application state** is owned by your Angular component or service. It is UI-only: sidebar visibility, active tab, selected message, form input values. It has nothing to do with the agent.

```typescript
// Defined once (e.g. in agent.ts):
// export const CHAT_AGENT = createAgentRef<ChatState>('chat');

@Component({
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ChatComponent {
  // --- Thread state (from injectAgent(), read-only) ---
  protected readonly agent = injectAgent(CHAT_AGENT);

  // Convenience computed values from thread state
  readonly messages = this.agent.messages;           // Signal<Message[]>
  readonly isLoading = this.agent.isLoading;         // Signal<boolean>
  readonly interrupted = this.agent.interrupt;       // Signal<AgentInterrupt | undefined>
...
<Callout type="warning" title="Do not mirror thread state into Angular signals">
...
├── Checkpoint 3  ← After call_model: { messages: [..., AIMessage("Here is what I found...")] }
└── (current)
```

### What This Means for Your Angular App

**Resumable threads** — If the user refreshes the page or closes the browser, the thread is still there. Pass the same `threadId` to `provideAgent()` and `injectAgent()` will restore the full conversation history automatically.
...
// (CHAT_AGENT = createAgentRef<ChatState>('chat'), declared in agent.ts)
// The factory form runs inside an injection context, so it can inject router
// services — a plain object literal at bootstrap cannot.
provideAgent(CHAT_AGENT, () => {
  ...
  return {
    ...
    threadId: signal(route.snapshot.params['threadId']),

    // New threadId auto-created for new conversations
    onThreadId: (id: string) => router.navigate(['/chat', id]),
  };
});

// component.ts
const agent = injectAgent(CHAT_AGENT);
...
For full checkpoint and time-travel patterns, see the [Persistence guide](/docs/langgraph/guides/persistence) and [Time Travel guide](/docs/langgraph/guides/time-travel).
...
def researcher_node(state: ResearchState) -> dict:
    # A node returns ONE dict, so accumulate across the loop and return once
    findings = []
    for step in state["plan"]:
        findings.append(search(step))
    return {
        "findings": findings,  # Appended via reducer
        "progress": state["progress"] + 80,
    }
```
...
export const RESEARCH_AGENT = createAgentRef<ResearchState>('research_agent');

// Configured globally in app.config.ts:
// provideAgent(RESEARCH_AGENT, { apiUrl: '...' });

// In your component:
protected readonly agent = injectAgent(RESEARCH_AGENT);
```
...
<CardGroup cols={2}>
  <Card title="Angular Signals" href="/docs/langgraph/concepts/angular-signals">
    How `injectAgent()` uses Angular Signals for zero-subscription reactive rendering.
  </Card>
