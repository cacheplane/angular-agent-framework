---
description: What a LangGraph graph is made of — state, nodes, edges, checkpoints — and how each piece reaches an Angular app through injectAgent()
---

# LangGraph Basics

LangGraph is a framework for building stateful AI agents as directed graphs. For an Angular developer building AI-powered applications, the sections below cover how LangGraph agents work and why `injectAgent()` is the natural bridge between your frontend and your agent backend.
...
Nodes do not replace state — they return updates that get **merged** into the existing state. For lists like messages, LangGraph uses reducers (like `operator.add`) to accumulate entries instead of overwriting.
</Callout>
...
The simplest possible agent — a chat model that takes messages and responds:
...
```typescript
const orchestrator = injectAgent();
...
This is why `injectAgent()` is the natural Angular companion for LangGraph.
...
<Step title="FetchStreamTransport">
Parses SSE chunks into typed stream events
</Step>
<Step title="`injectAgent()`">
...
agent.error()          // Signal<AgentError | undefined> — classified failure
...
You do not configure SSE, parse events, manage WebSocket connections, or handle reconnection. `injectAgent()` does all of that. You call `submit()` and read Signals — that is the entire API surface for your Angular code.
</Callout>
