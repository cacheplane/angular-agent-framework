...
`onThreadId` fires whenever the backend assigns a thread ID. The callback records it as the active thread and appends it to the list if it is new, which is the only bookkeeping the sidebar needs.
...
`switchThread()` also composes into a fork, which the example does not do: start a fresh thread and resubmit the messages you already have.
...
For a fork that branches from an earlier checkpoint rather than from the end of the conversation, see the [Time Travel guide](/docs/langgraph/guides/time-travel).
