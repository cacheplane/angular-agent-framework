...
Its system prompt restates the structural fact in words, telling the researcher that it cannot see the chat transcript and that its output is an internal brief for the parent to use.
...
The application config registers that ref with `provideAgent()` and adds `provideChat({})` for the chat composition. In your own application the two connection values are literals:
...
`status()` and `name` come off the `SubagentStreamRef`, which is why the mapped objects carry a called signal rather than the ref itself.
