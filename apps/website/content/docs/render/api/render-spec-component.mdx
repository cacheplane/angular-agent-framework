...
The example has no agent. The spec arrives from a local streaming simulator rather than a model, which keeps the moving parts down to the spec, the registry and the store. There is a `graph.py` in the capability, but this Angular application never calls it — its `app.config.ts` registers nothing but the render feature.
...
The context is a `computed` signal, so it is rebuilt when any input or the configuration changes. A component mounted inside the tree can inject it directly:
