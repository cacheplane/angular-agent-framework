...
An `AngularRegistry` object with two accessors:
...
Each entry can be a bare component class or a `{ component, fallback }` object. The `fallback` mounts while any resolved prop on the element is still `undefined` -- typically a `$state` binding that has not arrived yet. This is useful for streaming UI, where structure arrives before data:
...
<Callout type="warning" title="Validation is synchronous only">
The mount gate reads the schema result synchronously. A schema whose `validate()` returns a Promise does not gate the mount -- the element is treated as ready. Use a synchronous validator such as Zod.
</Callout>
