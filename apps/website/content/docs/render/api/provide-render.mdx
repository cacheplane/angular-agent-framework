...
Press play and watch the values appear: `hello world` comes out uppercased, `streaming` comes out reversed, an ISO timestamp comes out as a local date, and `7 x 6` comes out as `42`. None of those strings are in the spec. The spec asks for a function by name, and the four functions registered in `provideRender()` produce the text. Use the Spec tabs in the header to switch between three specs -- Text Transforms, Data Display, and Mixed Functions -- and each switch restarts the stream with a different mix of the same four functions.
...
| `handlers` | `Record<string, Handler>` | Named action handlers invoked when interactive elements fire |
