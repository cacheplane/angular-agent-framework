...
Every registered component is a plain standalone Angular component. `content` is the element's own prop from the spec; the five inputs under it are the framework inputs the renderer supplies for every element it mounts.
...
A registered value can be a bare component class or a full entry object. `defineAngularRegistry()` normalizes both into the same shape: a component, a guaranteed fallback, and the optional `schema` and `description` an entry may declare.
...
registry.getEntry('Text')?.component; // DemoTextComponent
