...
The current exported message union is (every envelope also carries `version: 'v0.9'`):
...
A surface has an id, catalog id, component map, data model, optional theme, and optional `sendDataModel` behavior.
...
`views()` produces a `ViewRegistry` -- the shape chat catalogs consume. `defineAngularRegistry()` produces the `AngularRegistry` that `<render-spec>` consumes directly. `toRenderRegistry()` converts the former into the latter, so a chat catalog and a direct render share one component map. Use `views()` when you are wiring components into chat; use `defineAngularRegistry()` when you are driving `<render-spec>` yourself. See the [Registry guide](/docs/render/guides/registry) for the low-level registry, and the [chat views guide](/docs/chat/guides/generative-ui) for the chat-facing path.

A2UI catalogs use the same underlying registry shape in chat. Component names such as `Text`, `Button`, `Card`, `Column`, `Row`, `TextField`, `CheckBox`, `ChoicePicker`, and `Slider` resolve to Angular components.
...
A2UI carries a client data model in protocol messages. Components use dynamic values — bare literals (strings, numbers, booleans, arrays), `{ path }` references into the data model, or typed client-side function calls. The chat-side surface store applies data model updates and gates component readiness until bound values resolve.
...
json-render actions are local render events. A rendered component calls `emit()`, and `RenderElementComponent` invokes the matching handler from the element's `on[event]` bindings.
...
If a surface's `createSurface` set `sendDataModel: true`, the action message includes `metadata.a2uiClientDataModel`.
