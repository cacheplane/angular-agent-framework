---
description: What @threadplane/render is, how it relates to @json-render/core, and the rendering pipeline that turns a JSON spec into a live Angular component tree.
---
...
You do not write imperative rendering logic. You describe your UI as a JSON spec and let the library handle the rest:
...
- **Recursive rendering** -- each element mounts via `NgComponentOutlet` and receives `childKeys` plus the full `spec`, so your component renders its own children with `<render-element>`
...
`defineAngularRegistry()` is the canonical constructor for driving `<render-spec>` directly. When you wire components into `@threadplane/chat` instead, you will see `views()` -- a chat-facing helper that produces a `ViewRegistry` chat converts to this same registry internally. Reach for `defineAngularRegistry()` here; reach for `views()` when feeding a chat catalog.
...
<CardGroup cols={2}>
  ...
  <Card title="Lifecycle Signals" href="/docs/render/guides/lifecycle">
