...
- Node telemetry runs only when application or adapter code explicitly calls a capture helper;
...
// Node/server helpers
import { disableTelemetry, captureEvent } from '@threadplane/telemetry/node';
```
...
Installing the package is inert: it has no install lifecycle hook and makes no network request. Node helpers check disable environment variables before an explicit capture call. They use a per-process anonymous ID and deterministic sampling based on that ID.

Node capture failures return a failure result or are swallowed by adapter helpers. Telemetry should not break startup or request handling.
...
- [Node Telemetry](/docs/telemetry/guides/node) - explicit capture helpers for server adapters.
