...
Node telemetry lives under `@threadplane/telemetry/node`. Installing it is inert. Events are sent only when application or server-adapter code calls a capture helper.
