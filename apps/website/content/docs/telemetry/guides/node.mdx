...
unless `TPLANE_TELEMETRY_INGEST_URL` is set.

Sampling uses `TPLANE_TELEMETRY_SAMPLE_RATE`. Invalid values fall back to `1`. Values are clamped to the range `0` to `1`.

Every sent event includes `sample_weight`.

## Postinstall script

The postinstall entry point reads package name and version from npm lifecycle environment variables or package.json, skips disabled environments, and sends `tplane:postinstall`.
...
When `DEBUG` includes `tplane:telemetry`, `tplane:*`, or `*`, the script prints the payload shape it attempted to send. It prints the normal install telemetry notice only when the ingest endpoint accepted the event.
...
describe('telemetry opt-out', () => {
  it('does not send when disabled', async () => {
    ...
    const result = await captureEvent('tplane:runtime_instance_created', { transport: 'langgraph' });
