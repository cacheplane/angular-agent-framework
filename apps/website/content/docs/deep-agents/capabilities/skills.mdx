---
title: Skills
description: SkillsMiddleware puts only a skill's frontmatter in the system prompt and leaves the body on the backend until a request matches.
---
...
Step 2 is the second stage of the disclosure: the margin table costs nothing until the `SKILL.md` sends the agent to it.
...
Nothing in the demo writes to the store after import, so `/skills/` stays stable for the whole run, even though the machinery serving it is the same `StoreBackend` a writable mount would use.
...
### Announcing the private state keys
...
<ExampleCode file="graph.py" region="visibility-middleware" title="graph.py — republishing skills_metadata and skills_load_errors" />

This is the same application-side shim the [memory capability](/docs/deep-agents/capabilities/memory) uses, and it is worth naming as such rather than mistaking it for a framework feature.
...
Rendering both halves in one column is the point, because the index is what the prompt paid for and the read paths are what the request actually cost.
