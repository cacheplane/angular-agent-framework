...
One security note: in both paths, only registered component names render — the registry is doing allowlist duty.
...
A submit action always goes back to the agent as a structured action message; create the surface with `sendDataModel` and the current data model rides along.
