...
This capability demonstrates a deep agent executing code and shell commands inside an isolated sandbox environment using the `@threadplane/chat` Angular component library. The `<chat-debug>` component surfaces every sandbox invocation — the code submitted, the execution environment, stdout/stderr, and exit codes — giving developers complete visibility into agent-driven code execution.
