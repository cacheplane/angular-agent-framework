  ...
  template: `
    <div class="inline-flex items-center gap-2 rounded-lg px-3 py-1.5 my-1"
      ...
      <span class="rounded-full px-2 py-0.5 text-xs font-semibold"
            style="background: rgba(74, 222, 128, 0.15); color: var(--tplane-chat-success);">
        calculator
      </span>
      <span class="text-sm font-mono" style="color: var(--tplane-chat-text);">
        {{ expression() }}
      </span>
      <span class="text-sm font-semibold" style="color: var(--tplane-chat-text);">
        = {{ result() }}
      </span>
    </div>
  `,
