...
This capability demonstrates cross-thread memory using LangGraph's persistent memory store and the `@threadplane/chat` Angular component library. Facts and preferences written by the agent in one thread are automatically recalled in subsequent threads, giving the user a coherent long-term experience across sessions.
